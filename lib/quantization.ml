...
module FP32_to_FP8 (F : FP8) : Quant = struct
  ...
  let min_exp =
    let me = -((1 lsl (exponent - 1)) - 2) in
    Base.Int32.of_int_exn me
  ;;

  let round_bitwise target =
    let open Unsigned.UInt32 in
    let mask = shift_left (of_int 1) (23 - mantissa) in
    let mask = sub mask (of_int 1) in
    let rand_prob = shift_left (of_int 1) (23 - mantissa - 1) in
    let add_r = add target rand_prob in
    logand add_r (lognot mask)
  ;;

  let clip_exponent old_num q_num =
    let max_exp = exponent - 1 in
    let open Unsigned.UInt32 in
    let open Infix in
    let clip_up max_exp =
      let max_man = ((of_int (-1) lsl 9) lsr 9) lsr Base.Int.(23 - mantissa) in
      let max_num = (max_exp lsr 23) lor max_man in
      let old_sign = (old_num lsr 31) lsl 31 in
      old_sign lor max_num
    in
    let clip_quant () =
      let q_exp_store = ((q_num lsl 1) lsr 1) lsr 23 |> to_int32 in
      let max_exp_store = of_int 1 lsl max_exp in
  ...
  let quantize_normal target =
    let target = Unsigned.UInt32.of_int32 target in
  ...
  let quantize ~fp32 =
    ...
    if is_subnormal then quantize_normal target |> float_of_bits else fp32
  ;;
end
